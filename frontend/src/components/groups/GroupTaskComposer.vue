<script setup>
import {
  AlignLeft,
  CalendarDays,
  Check,
  ChevronDown,
  Circle,
  ClipboardPlus,
  Clock3,
  Flag,
  GripVertical,
  RefreshCw,
  UserRoundCheck,
  UserRoundX,
  X
} from '@lucide/vue'
import { computed, onBeforeUnmount, onMounted, ref, watch } from 'vue'
import { groupInitial, groupRoleLabel } from '../../utils/groups'

const props = defineProps({
  open: Boolean,
  groupName: {
    type: String,
    default: '当前工作组'
  },
  members: {
    type: Array,
    default: () => []
  },
  submitting: Boolean,
  valid: Boolean,
  errorMessage: {
    type: String,
    default: ''
  },
  priorityOptions: {
    type: Array,
    default: () => []
  },
  dueLabel: {
    type: String,
    default: '未设置截止时间'
  },
  hasDueValue: Boolean,
  duePresets: {
    type: Array,
    default: () => []
  },
  isDuePresetActive: {
    type: Function,
    required: true
  },
  customDueOpen: Boolean,
  dateParts: {
    type: Object,
    required: true
  },
  timeParts: {
    type: Object,
    required: true
  },
  panelWidth: {
    type: Number,
    required: true
  },
  panelBounds: {
    type: Object,
    required: true
  },
  resizing: Boolean
})

const emit = defineEmits([
  'close',
  'submit',
  'clear-error',
  'priority-change',
  'clear-due',
  'apply-due-preset',
  'toggle-custom-due',
  'update-date-part',
  'update-time-part',
  'resize-start',
  'resize-reset',
  'resize-keydown'
])
const title = defineModel('title', { type: String, default: '' })
const description = defineModel('description', { type: String, default: '' })
const priority = defineModel('priority', { type: String, default: 'MEDIUM' })
const assigneeId = defineModel('assigneeId', { type: [String, Number], default: '' })
const assigneeMenuOpen = ref(false)
const assigneeControlRef = ref(null)
const selectedAssignee = computed(() => props.members.find((member) => (
  String(member.userId) === String(assigneeId.value)
)) || null)
const vFocus = {
  mounted(element) {
    element.focus()
    element.select()
  }
}

function selectAssignee(member) {
  assigneeId.value = member?.userId ?? ''
  assigneeMenuOpen.value = false
  emit('clear-error')
}

function closeAssigneeMenuOnOutsidePointer(event) {
  if (assigneeMenuOpen.value && !assigneeControlRef.value?.contains(event.target)) {
    assigneeMenuOpen.value = false
  }
}

watch(() => props.open, (open) => {
  if (!open) {
    assigneeMenuOpen.value = false
  }
})

onMounted(() => document.addEventListener('pointerdown', closeAssigneeMenuOnOutsidePointer))
onBeforeUnmount(() => document.removeEventListener('pointerdown', closeAssigneeMenuOnOutsidePointer))
</script>

<template>
  <aside v-if="open" class="detail-panel create-panel group-task-create-panel">
    <div
      class="detail-resize-handle"
      role="separator"
      aria-label="调整团队任务创建栏宽度"
      aria-orientation="vertical"
      :aria-valuemin="panelBounds.min"
      :aria-valuemax="panelBounds.max"
      :aria-valuenow="panelWidth"
      tabindex="0"
      title="拖动调整创建栏宽度，双击恢复默认"
      @pointerdown="emit('resize-start', $event)"
      @dblclick="emit('resize-reset')"
      @keydown="emit('resize-keydown', $event)"
    >
      <GripVertical :size="15" />
      <span v-if="resizing">{{ panelWidth }} px</span>
    </div>

    <div class="detail-panel-scroll create-panel-scroll">
      <header class="detail-header create-panel-header">
        <div class="detail-heading-copy">
          <p>创建团队任务</p>
          <span>{{ groupName }}</span>
        </div>
        <button type="button" class="icon-button" aria-label="关闭团队任务创建栏" :disabled="submitting" @click="emit('close')">
          <X :size="18" />
        </button>
      </header>

      <form class="create-detail-form group-task-create-form" @submit.prevent="emit('submit')">
        <div class="group-task-create-intro">
          <span><ClipboardPlus :size="21" /></span>
          <div>
            <small>TEAM TASK</small>
            <strong>安排新的协作事项</strong>
          </div>
          <b><Circle :size="11" /> 待办</b>
        </div>

        <label class="create-title-field group-task-title-field">
          <span>任务标题</span>
          <input
            v-focus
            v-model="title"
            type="text"
            maxlength="100"
            placeholder="团队接下来要完成什么？"
            @input="emit('clear-error')"
          />
          <small>{{ title.length }} / 100</small>
        </label>

        <section class="create-form-section group-assignee-section">
          <div class="create-section-heading">
            <div>
              <span class="create-section-icon assignee-icon"><UserRoundCheck :size="16" /></span>
              <span><strong>负责人</strong><small>{{ selectedAssignee?.username || '暂不分配' }}</small></span>
            </div>
          </div>

          <div ref="assigneeControlRef" class="group-assignee-control">
            <button
              class="group-assignee-trigger"
              type="button"
              :class="{ open: assigneeMenuOpen }"
              :aria-expanded="assigneeMenuOpen"
              aria-haspopup="listbox"
              @click="assigneeMenuOpen = !assigneeMenuOpen"
            >
              <span class="group-assignee-avatar" :class="{ empty: !selectedAssignee }">
                {{ selectedAssignee ? groupInitial(selectedAssignee.username) : '—' }}
              </span>
              <span>
                <strong>{{ selectedAssignee?.username || '暂不分配负责人' }}</strong>
                <small>{{ selectedAssignee ? groupRoleLabel(selectedAssignee.role) : '未指定成员' }}</small>
              </span>
              <ChevronDown :size="16" />
            </button>

            <Transition name="property-reveal">
              <div v-if="assigneeMenuOpen" class="group-assignee-menu" role="listbox" aria-label="选择任务负责人">
                <button
                  type="button"
                  role="option"
                  :aria-selected="!selectedAssignee"
                  :class="{ active: !selectedAssignee }"
                  @click="selectAssignee(null)"
                >
                  <span class="group-assignee-avatar empty"><UserRoundX :size="15" /></span>
                  <span><strong>暂不分配</strong><small>无负责人</small></span>
                  <Check v-if="!selectedAssignee" :size="14" />
                </button>

                <button
                  v-for="member in members"
                  :key="member.userId"
                  type="button"
                  role="option"
                  :aria-selected="String(member.userId) === String(assigneeId)"
                  :class="{ active: String(member.userId) === String(assigneeId) }"
                  @click="selectAssignee(member)"
                >
                  <span class="group-assignee-avatar">{{ groupInitial(member.username) }}</span>
                  <span><strong>{{ member.username }}</strong><small>{{ groupRoleLabel(member.role) }}</small></span>
                  <Check v-if="String(member.userId) === String(assigneeId)" :size="14" />
                </button>
              </div>
            </Transition>
          </div>
        </section>

        <section class="create-form-section">
          <div class="create-section-heading">
            <div><span class="create-section-icon priority-icon"><Flag :size="16" /></span><strong>优先级</strong></div>
          </div>
          <div class="create-choice-grid priority-choice-grid" aria-label="团队任务优先级">
            <button
              v-for="option in priorityOptions"
              :key="option.value"
              type="button"
              :class="[option.tone, { active: priority === option.value }]"
              @click="priority = option.value; emit('priority-change', option.value)"
            >
              <Flag :size="14" />
              <span>{{ option.label }}</span>
            </button>
          </div>
        </section>

        <section class="create-form-section">
          <div class="create-section-heading">
            <div><span class="create-section-icon description-icon"><AlignLeft :size="16" /></span><strong>描述</strong></div>
            <small>{{ description.length }} / 1000</small>
          </div>
          <textarea
            v-model="description"
            maxlength="1000"
            rows="5"
            placeholder="补充交付目标、协作背景或注意事项"
            @input="emit('clear-error')"
          ></textarea>
        </section>

        <section class="create-form-section create-due-section group-task-due-section">
          <div class="create-section-heading">
            <div>
              <span class="create-section-icon due-icon"><CalendarDays :size="16" /></span>
              <span><strong>截止时间</strong><small>{{ dueLabel }}</small></span>
            </div>
            <button v-if="hasDueValue" type="button" @click="emit('clear-due')">清除</button>
          </div>

          <div class="create-due-presets">
            <button
              v-for="preset in duePresets"
              :key="preset.value"
              type="button"
              :class="{ active: isDuePresetActive(preset.value) }"
              @click="emit('apply-due-preset', preset.value)"
            >
              <strong>{{ preset.label }}</strong>
              <small>{{ preset.meta }}</small>
            </button>
          </div>

          <button
            class="due-custom-toggle"
            type="button"
            :class="{ open: customDueOpen }"
            :aria-expanded="customDueOpen"
            @click="emit('toggle-custom-due')"
          >
            <span><Clock3 :size="15" /> 自定义日期与时间</span>
            <ChevronDown :size="15" />
          </button>

          <Transition name="property-reveal">
            <div v-if="customDueOpen" class="due-custom-fields">
              <div class="date-part-grid create-date-grid" aria-label="团队任务截止日期">
                <label v-for="part in ['year', 'month', 'day']" :key="part">
                  <span>{{ { year: '年', month: '月', day: '日' }[part] }}</span>
                  <input
                    :value="dateParts[part]"
                    inputmode="numeric"
                    :maxlength="part === 'year' ? 4 : 2"
                    :placeholder="{ year: '2026', month: '08', day: '31' }[part]"
                    @input="emit('update-date-part', part, $event.target.value)"
                  />
                </label>
              </div>

              <div class="time-part-grid create-time-grid" aria-label="团队任务截止时间">
                <label>
                  <span>时</span>
                  <input :value="timeParts.hour" inputmode="numeric" maxlength="2" placeholder="23" @input="emit('update-time-part', 'hour', $event.target.value)" />
                </label>
                <i>:</i>
                <label>
                  <span>分</span>
                  <input :value="timeParts.minute" inputmode="numeric" maxlength="2" placeholder="59" @input="emit('update-time-part', 'minute', $event.target.value)" />
                </label>
              </div>
            </div>
          </Transition>
        </section>

        <p v-if="errorMessage" class="notice error create-panel-error" role="alert">{{ errorMessage }}</p>

        <footer class="create-panel-actions">
          <button type="button" :disabled="submitting" @click="emit('close')">取消</button>
          <button class="primary-button" type="submit" :disabled="submitting || !valid">
            <RefreshCw v-if="submitting" class="spin-icon" :size="16" />
            <ClipboardPlus v-else :size="17" />
            <span>{{ submitting ? '创建中...' : '创建团队任务' }}</span>
          </button>
        </footer>
      </form>
    </div>
  </aside>
</template>
